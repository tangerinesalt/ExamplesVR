...
public class ModelMeshEditor : MonoBehaviour {
    ...
    public Vector3 ControlPointSize=new Vector3(1,1,1);
    public bool RestoreOnDisabled = true;
    ...
    public void CreateEditorPoint(){
        ...
        RecordPositionList = new List<Vector3>(positionList);
        ...
        foreach (string key in pointmap.Keys)
        {
            ...
            editorpoint.transform.localScale = ControlPointSize;
    ...
    {
        Debug.Log("OnDisable");
        if (RestoreOnDisabled)
        {
            // for (int i = 0; i < positionList.Count; i++)
            // {
            //     if (positionList[i] != RecordPositionList[i])
            //     {
            //         Debug.Log("positionList != RecordPositionList");
            //         positionList[i] = RecordPositionList[i];
            //     }
            // }
            positionList = new List<Vector3>(RecordPositionList);
            mesh.vertices = positionList.ToArray();
            mesh.RecalculateNormals();
        }
    }
